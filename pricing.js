(() => {
  ...
  function createUi() {
    ...
    panel.innerHTML = `
      <div class="recommendation-heading">
        ...
        <p>Revenue-first recommendations from the same prior-year and matching-weekday dates used by the forecast.</p>
      </div>
  ...
  function comparisonKeys(byDate, targetKey, count) {
    const target = parseKey(targetKey);
    if (!target) return [];
    const priorKey = previousYearKey(targetKey);
    const keys = [...byDate.keys()].sort();
    const weekdays = keys
      .filter(key => key < targetKey && key !== priorKey && parseKey(key)?.getDay() === target.getDay())
      .sort((a, b) => b.localeCompare(a))
      .slice(0, count);
    return [priorKey && byDate.has(priorKey) ? priorKey : null, ...weekdays].filter(Boolean);
  }
  ...
  function buildRecommendations(byDate, historyKeys, targetKey) {
    ...
    historyKeys.forEach(dayKey => {
      aggregateOptions(byDate.get(dayKey) || []).forEach(option => {
        ...
        if (!prices.has(key)) prices.set(key, { amount: option.amount, tickets: 0, revenue: 0, dayKeys: new Set() });
    ...
    return [...types.entries()].map(([type, priceMap]) => {
      ...
      const observed = (observedByType.get(type) || []).sort((a, b) => b.count - a.count || b.amount - a.amount);
      const primary = observed[0] || null;
      const current = primary ? candidates.find(candidate => priceKey(candidate.amount) === priceKey(primary.amount)) : null;
      const [confidenceLabel, confidenceClass] = confidence(recommended);
      return { type, candidates, recommended, observed, primary, current, confidenceLabel, confidenceClass };
  ...
  function changeHtml(row) {
    ...
    if (priceKey(row.primary.amount) === priceKey(row.recommended.amount)) {
      return `<strong>Keep ${cash(row.primary.amount)}</strong><span class="forecast-subtext">Highest revenue per tested day</span>`;
    }
    if (!row.current) {
    ...
    return `<strong>${signedMoney(revenueDelta)} / tested day</strong><span class="forecast-subtext">${cash(row.primary.amount)} → ${cash(row.recommended.amount)} · ${signedCars(ticketDelta)} cars/day</span>`;
  }

  function testedHtml(row) {
    return `<div class="price-test-list">${[...row.candidates].sort((a, b) => a.amount - b.amount).map(candidate => {
      const best = priceKey(candidate.amount) === priceKey(row.recommended.amount);
      return `<span class="price-test-chip ${best ? "recommended" : ""}"><strong>${cash(candidate.amount)}</strong> · ${cash(candidate.dailyRevenue)}/day · ${candidate.dailyTickets.toFixed(1)} cars · ${candidate.days} day${candidate.days === 1 ? "" : "s"}</span>`;
    }).join("")}</div>`;
  }
  ...
  function render() {
    ...
    const historyKeys = comparisonKeys(byDate, targetKey, count);
    if (!historyKeys.length) {
      notice.className = "recommendation-notice warning";
      notice.textContent = "No prior-year or matching-weekday comparison dates are available for this lot.";
    ...
    notice.textContent = "Each exact price is ranked by average revenue on comparison dates where it sold at least one ticket. For example, 26 cars at $30 ($780) beats 30 cars at $25 ($750). Low-confidence results need more test days because the CSV does not show prices that were offered but sold zero tickets.";
